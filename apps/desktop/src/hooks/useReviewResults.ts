...
import { getTicketReviewHistory, triggerReview } from '../services/api';
import type { ReviewResultEntry, ReviewHistoryResponse, ReviewResultMessage, IncomingMessage } from '../types/api';
...
/**
 * Type guard for review:result messages
 */
export function isReviewResultMessage(msg: IncomingMessage): msg is ReviewResultMessage {
  return msg.type === 'review:result';
}

/**
 * Hook to fetch and subscribe to review results for a ticket
 */
export function useReviewResults(ticketId: string | undefined, lastMessage: IncomingMessage | null) {
  const queryClient = useQueryClient();

  // Fetch review history from API
  const query = useQuery({
    queryKey: reviewResultKeys.history(ticketId!),
    queryFn: () => getTicketReviewHistory(ticketId!),
    enabled: !!ticketId,
  });

  // Handle WebSocket messages for real-time updates
  useEffect(() => {
    if (!lastMessage || !isReviewResultMessage(lastMessage)) return;
...
export function useTriggerReview(ticketId: string | undefined) {
  ...
  return useMutation({
    mutationFn: (sessionId: string) => triggerReview(sessionId),
    onSuccess: () => {
